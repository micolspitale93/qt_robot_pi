...
class VoiceInput:
	...
	self.state = "Start"
        self.audio_publisher = rospy.Publisher(self.base_topic+"/microphone_input", AudioData, queue_size=5)
	self.state_publisher = rospy.Publisher(self.base_topic+"/state", String, queue_size=10)
	rospy.Subscriber(self.base_topic+"/speaker_state", String, self.handle_speaker_state)
    ...
    def get_record_device_index(self):
	    ...
	    if device['name'] == 'ReSpeaker 4 Mic Array (UAC1.0): USB Audio (hw:1,0)':
                rospy.loginfo("Found device 'Mic Array' at index %d" % i)
...
def main():
    try:
        voice_input = VoiceInput(node_name="microphone_node", robot_name = "qt_robot")
